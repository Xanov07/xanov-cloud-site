...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- AGENT COMPARE ---
    const compareCbs = document.querySelectorAll('.agent-compare-cb');
    const compareBtn = document.getElementById('compareBtn');
    const compareChecks = document.getElementById('compareChecks');
    const compareHint = compareChecks ? compareChecks.querySelector('.agents__compare-hint') : null;
    ...
    const agentData = {
        sales: {
            ...
            price: 'от $300',
            channels: '1–3+ канала',
            features: 'Квалификация лидов, сбор заявок, логика под бизнес',
            support: '30 дней включено',
            upgrade: 'CRM, обучение на данных'
        },
        analytics: {
            name: 'AI-аналитик данных',
            price: 'от $400',
            channels: 'Таблицы, базы данных',
            features: 'Отчёты, прогнозы, аномалии',
            support: '30 дней включено',
            upgrade: 'Интеграция с BI-системами'
        },
        content: {
            name: 'AI-агент контента',
            price: 'от $200',
            channels: 'Соцсети, блоги',
            features: 'Посты, публикация по расписанию, тон бренда',
            support: '30 дней включено',
            upgrade: 'До 50 шаблонов контента'
        }
    };

    let selectedAgents = [];

    compareCbs.forEach(cb => {
        cb.addEventListener('change', () => {
            const name = cb.dataset.name;
            const card = cb.closest('.agent-card');
            const agentId = card ? card.dataset.agentId : null;

            if (cb.checked) {
                if (selectedAgents.length >= 3) { cb.checked = false; return; }
                selectedAgents.push({ name, id: agentId });
            } else {
                selectedAgents = selectedAgents.filter(a => a.name !== name);
            }

            if (compareHint) compareHint.style.display = selectedAgents.length ? 'none' : '';

            const tags = compareChecks.querySelectorAll('.agents__compare-tag');
            tags.forEach(t => t.remove());
            selectedAgents.forEach(a => {
                const tag = document.createElement('span');
                tag.className = 'agents__compare-tag';
                tag.textContent = a.name;
                compareChecks.appendChild(tag);
            });

            compareBtn.disabled = selectedAgents.length < 2;
        });
    });
    ...
    if (compareBtn) {
        compareBtn.addEventListener('click', () => {
            ...
            const rows = [
                { label: 'Агент', key: 'name' },
                { label: 'Цена', key: 'price' },
                { label: 'Каналы', key: 'channels' },
                { label: 'Возможности', key: 'features' },
                { label: 'Поддержка', key: 'support' },
                { label: 'Апгрейд', key: 'upgrade' },
            ];
            const cols = selectedAgents.map(a => agentData[a.id] || {});
            compareTable.innerHTML = rows.map((row, i) =>
                `<div class="compare-row${i === 0 ? ' compare-row--header' : ''}">
                    <span class="compare-label">${row.label}</span>
                    ${cols.map(c => `<span class="compare-val">${c[row.key] || '—'}</span>`).join('')}
                </div>`
            ).join('');
